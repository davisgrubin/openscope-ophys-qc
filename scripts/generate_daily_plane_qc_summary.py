#!/usr/bin/env python3
"""Generate a one-plane QC summary PDF across all sessions from a given day."""
...
from mesoscope_qc_reports import DEFAULT_MAX_FRAMES, DEFAULT_MASK_LIMIT, build_session_report_context, plot_plane_qc_overview
...
def main() -> int:
    ...
    parser.add_argument("--plane", required=True, help="Plane name to summarize, e.g. VISp_0.")
    parser.add_argument("--output", default="outputs/qc_summaries/daily_plane_qc_summary.pdf", type=Path)
    ...
    with PdfPages(args.output) as pdf:
        ...
        title.suptitle(f"Daily plane QC summary: {args.date} | {args.plane}", fontsize=18, fontweight="bold", y=0.94)
        ...
        for session_source in matches:
            ...
            if args.plane not in ctx.planes:
                continue
            fig = plot_plane_qc_overview(ctx, args.plane)
